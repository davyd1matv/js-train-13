...
function complexConvert(data) {
  ...
  for (const element of key) {
    if (typeof el[element] === "number") {
      obj[element] = el[element] += 1;
    } else {
      obj[element] = JSON.parse(data)[element];
    }
  }
  return obj;
...
function searchParamsURL(url) {
  ...
  const URLSearchParams = URL1.searchParams;
...
function appendSearchParam(url, key, value) {
  const urlObj = new URL(url);
  urlObj.searchParams.append(value, key);
  return urlObj.href;
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
...
function checkURLParameters(url, params) {
  ...
  for (let element of params) {
    // if (urlObj.searchParams.has(element)) {
    result[element] = urlObj.searchParams.has(element);
    // result.searchParams.set(element[0], element[1]);
    // }
  }
  return result;
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
...
function processUrl(url, options) {
  const urlObj = new URL(url);
  //   if (options.searchParams.has(searchParams)) {

  if (options.searchParams) {
    for (const element in options.searchParams) {
      urlObj.searchParams.set(element, options.searchParams[element]);
    }
  }
  if (options.protocol) {
    urlObj.protocol = options.protocol;
    // urlObj.searchParams.set(urlObj.protocol, options.protocol);
  }
  if (options.host) {
    urlObj.host = options.host;
    // urlObj.searchParams.set(urlObj.host, options.host);
  }
